 ...
 *        node scripts/generate-batch-migration.mjs --taxonomy-corrections
...
/** Ratified WO-4 corrections to shipped taxonomy side-cars (not movement.pattern). */
const TAXONOMY_CORRECTIONS = [
  ['Cable Rope Overhead Triceps Extension', 'push', 'accessory'],
  ['Triceps Pushdown', 'push', 'accessory'],
];
...
const seededNames = new Set(Object.values(manifest.slots).flat());
...
if (process.argv.includes('--taxonomy-corrections')) {
  const existing = readdirSync(SCHEMA_DIR).find((f) => /^\d{3}_taxonomy_corrections\.sql$/.test(f));
  if (existing !== undefined) {
    console.log(`nothing to do: ${existing} already carries the ratified taxonomy corrections`);
    process.exit(0);
  }
  const errors = [];
  for (const [name, , to] of TAXONOMY_CORRECTIONS) {
    const staged = doc.movements.find((m) => m.name === name);
    if (!seededNames.has(name)) errors.push(`not a seeded movement: ${name}`);
    if (staged?.pattern !== to) errors.push(`staging category for ${name} must be ${to}`);
  }
  if (errors.length > 0) {
    console.error('ABORT:\n  ' + errors.join('\n  '));
    process.exit(1);
  ...
  const out = join(SCHEMA_DIR, `${slot}_taxonomy_corrections.sql`);
  const lines = [
    ...
    '',
    ...TAXONOMY_CORRECTIONS.flatMap(([name, from, to]) => [
      `UPDATE movement_taxonomy SET category = ${q(to)}`,
      `WHERE movement_id = (SELECT movement_id FROM movement WHERE name = ${q(name)});`,
      `-- Prior shipped category: ${from}.`,
      '',
    ]),
  ];
  writeFileSync(out, lines.join('\n'));
  console.log(`wrote ${out}: ${TAXONOMY_CORRECTIONS.length} taxonomy corrections`);
  console.log(`NEXT: import m${slot} in packages/core-db/src/migrations.ts and update migration/store gates.`);
  process.exit(0);
}
