# Handover — 2026-08-14 Longitudinal Deterministic Verification (rev. 4)

Owner: Francis Pike
Phase: test-only longitudinal verification (HERMES_WORK_ORDER_LONGITUDINAL_VERIFICATION.md, HERMES_AMENDMENT_OPUS_P2_CLOSEOUT.md, & AGENT.md, 2026-08-14)
...
- Checkpoint 4 (P2 closeout repair): the commit containing this handover. The audit range is `f6dce82002890c6e230670fe7708addacd350132..<checkpoint4>` (full range `a38ab102b9b01cebdc91c20214fed1560207cd39..<checkpoint4>`).
...
| File | Change |
|---|---|
| `packages/inference/test/verify_longitudinal_bounds.mjs` | Repaired `familyB` return shape (`variants`, `weeklyEvaluations`) to supply derived summary counts; all P2-1..P2-8 implementations fully retained and verified. |
| `tools/autopilot-sim/HANDOVER_2026-08-14_LONGITUDINAL_BOUNDS.md` | Updated to rev. 4 handover. |
...
- Evaluations: 68 total weekly evaluations (A: 16 time-invariant, B: 20 time-invariant across 5 variants, C: 16 calendar-advancing, D: 16 calendar-advancing). 32 weeks advance real production session dates; 36 evaluations are pure time-invariant routine microcycles. All summary counts derived from runtime trace arrays.
...
- **P2-6 (Truthful evaluation/week terminology):** Replaced ambiguous "68 simulated weeks" with 68 total weekly evaluations: 32 calendar-advancing generated weeks (C & D) and 36 time-invariant routine microcycle evaluations (A & B). All summary counts (`totalWeeklyEvaluations`, `calendarAdvancingWeeks`, `timeInvariantWeeklyEvaluations`, `weeklyEvaluations`, `dateAdvancingWeeks`) are derived dynamically from trace structures.
...
## 6. Gate results (rev. 4, in work-order order)
...
| `git diff --check f6dce82002890c6e230670fe7708addacd350132..HEAD` | 0 |
...
`.build`, ONNX assets, `node_modules`, and untracked `AGENT.md` cannot leak in.
...
`f6dce82002890c6e230670fe7708addacd350132..<checkpoint4>` and full range
`a38ab102b9b01cebdc91c20214fed1560207cd39..<checkpoint4>`. Reproduce all §6
gates from a clean worktree at the frozen parent. Confirm: only the two
listed files changed relative to `f6dce82`; `package.json` is byte-identical;
