...
exports.UNSTAGED_SLEEP_EFFICIENCY = void 0;
exports.aggregateDaily = aggregateDaily;
// androidx.health SleepStageType values that count as NOT asleep.
const AWAKE_STAGES = new Set([
    1, // AWAKE
    3, // OUT_OF_BED
    7, // AWAKE_IN_BED
]);
...
exports.UNSTAGED_SLEEP_EFFICIENCY = 0.92;
 ...
 */
function aggregateDaily(hrv, rhr, sleep, localDateOf) {
    const days = new Map();
    const dayOf = (iso) => {
        let date;
        try {
    ...
    for (const r of hrv ?? []) {
        if (typeof r?.heartRateVariabilityMillis !== 'number')
            continue;
        const v = r.heartRateVariabilityMillis;
        if (!Number.isFinite(v) || v <= 0 || v > 500)
            continue; // physiology bounds
    ...
    for (const r of rhr ?? []) {
        ...
        if (!Number.isFinite(v) || v < 20 || v > 150)
            continue; // hrv_daily CHECK domain
        const acc = dayOf(r.time);
        if (acc === null)
            continue;
        acc.rhrSum += v;
    ...
    for (const s of sleep ?? []) {
        ...
        if (stages.length > 0) {
            for (const st of stages) {
                ...
                if (stMin <= 0)
                    continue;
                if (AWAKE_STAGES.has(st.stage))
                    continue;
                acc.asleep += stMin;
                if (st.stage === STAGE_DEEP)
                    acc.deep += stMin;
                else if (st.stage === STAGE_REM)
                    acc.rem += stMin;
                else if (st.stage === STAGE_LIGHT)
                    acc.light += stMin;
            }
        }
        else {
            acc.asleep += duration * exports.UNSTAGED_SLEEP_EFFICIENCY;
        }
    }
    const out = [];
    for (const [date, acc] of [...days.entries()].sort(([a], [b]) => (a < b ? -1 : 1))) {
        ...
        if (rmssd === null && restingHr === null && inBed === null)
            continue;
        out.push({
            date,
            rmssdMs: rmssd,
