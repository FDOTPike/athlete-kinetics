...
const b2 = freshDb();
for (let i = 0; i < MIGRATIONS.length - 3; i += 1) b2.executeSync(MIGRATIONS[i]); // skip 016..018
...
check('016+017 seeds arrived: 96 movements', Number(b2.raw.prepare('SELECT COUNT(*) c FROM movement').get().c) === 96);
