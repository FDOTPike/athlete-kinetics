...
const b2 = freshDb();
for (let i = 0; i < MIGRATIONS.length - 4; i += 1) b2.executeSync(MIGRATIONS[i]); // skip 016..019
...
check('016+017+019 seeds arrived: 109 movements', Number(b2.raw.prepare('SELECT COUNT(*) c FROM movement').get().c) === 109);
