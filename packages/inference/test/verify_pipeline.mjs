...
// ---------------------------------------------------------------------------
{
  ...
  check('the acceptance database is the real 001-058 chain, not a trimmed subset', () => {
    assert.equal(fullChain[0], '001_mechanical_input.sql');
    assert.equal(fullChain[fullChain.length - 1], '058_suspension_episode.sql');
    assert.equal(fullChain.length, 57, `applied ${fullChain.length} migrations`);
