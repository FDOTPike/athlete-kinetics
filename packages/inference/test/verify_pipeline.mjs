...
// ---------------------------------------------------------------------------
{
  ...
  check('the acceptance database is the real 001-062 chain, not a trimmed subset', () => {
    assert.equal(fullChain[0], '001_mechanical_input.sql');
    assert.equal(fullChain[fullChain.length - 1], '062_suspension_sidecar_immutability.sql');
    assert.equal(fullChain.length, 61, `applied ${fullChain.length} migrations`);
