...
- `isWellFormedBackupContainer` now decodes the ciphertext with the strict bounded Base64 decoder and requires at least `MIN_WELL_FORMED_CIPHERTEXT_BYTES` (700) decoded ciphertext bytes. AES-GCM ciphertext is the encrypted archive plaintext plus the 16-byte tag. That plaintext is JSON that carries every athlete database as Base64 text, and every archive carries at least one database. One minimum 512-byte SQLite page is 684 Base64 characters, each one byte of plaintext, so the plaintext is at least 684 bytes and the ciphertext at least 684 + 16 = 700 bytes. `====`, non-alphabet text, interior padding and short ciphertext are rejected.
- `reconcileRecoveryPublication` keeps the same selection order: `pikeMethods-recovery-current.pmbak.previous`, then the retained `pikeMethods-recovery-current.pmbak`, then `pikeMethods-recovery-current.pmbak.new`. A well-formed candidate that is not selected is moved to `pikeMethods-recovery-current.pmbak.alternate-<sha256>`, never deleted. A byte-identical preserved copy makes the source redundant. Only structurally malformed rotation files are removed.
