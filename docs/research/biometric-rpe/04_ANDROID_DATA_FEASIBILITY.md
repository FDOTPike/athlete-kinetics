...
3. **Rest-interval HR is not a recovery measurement.** Inter-set HR decays under partial vagal reactivation whose timescale (seconds–minutes) no Health Connect record the app reads captures; treating "HR at next set start" as recovery would be an invented construct. (**Corrected per third audit:** Health Connect *does* support timestamped `HeartRateRecord` samples during exercise, queryable over a session's time range [D10] — but the app does not request or read them, and vendor write coverage for strength work is unverified. The limitation is the app's ingestion, not the platform's capability.)
