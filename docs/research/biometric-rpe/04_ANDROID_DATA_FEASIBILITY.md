...
- Continuous per-second HR ingestion: a **bounded worst-case estimate**, not a measurement — 3600 samples/h × ~16–32 B/sample (timestamp + value) ≈ 58–115 KB/h of raw samples; a 2-hour session with a 30-minute in-memory processing buffer and JSON/JS-object overhead (each sample as a JS object: order 100–200 B) lands in the **~5–15 MB** working-set range per active session, plus GC churn from per-sample object allocation; sustained background capture across a day would multiply this. The honest statement: the exact cost on the 4 GB target is **unknown until measured** — the release contract's own method is measured device evidence [R06] — so any future streaming design must begin with a device measurement gate, and the compaction boundary (native adapter in, pure daily aggregation out) must not move [R01].
