...
- **Five** manifest rows are partially verified (`S05`, `S06`, `S21`, `S22`, `S23` — flagged in-row; no numeric claim rests on them). *`S10` was listed here in earlier rounds and is **SUPERSEDED BY ROUND 3**: its full record (n = 17 healthy young adults, 12 M / 5 F, age 26.53±2.70) was supplied and the row is now fully verified.* This count is mechanically derived from the CSV and agrees with §3. **Round-2 disclosure:** the first remediation's three new literature rows were themselves wrong (S25 authors; S26 title/authors; S27 issue) and [S28] was missed — fixed in the second remediation; counting round 3's residual findings, this executor's citation verification has now failed across **four** rounds, and every row must be independently re-verified. *(This sibling line was missed by both the B-3 and P2-5 sweeps; corrected per Antigravity Round 2, P2-NEW-1 — the third time a “complete” sweep on this package left a sibling standing.)*
...
### Round 2 re-audit (target `920f49f0a7454714624f62056e3c8a155a6302ec`) — verdict REQUEST CHANGES

The panel re-audited the remediation above. **Closure: 16 FIXED, 4 PARTIAL, 0 REGRESSED.** Lanes A1, A3, A5 and A6 returned PASS; A2 and A4 returned REQUEST CHANGES. Two new P1s and three new P2s, all dispositioned here:

| # | Finding | Disposition |
|---|---|---|
| P1-NEW-1 | **Phantom-field starvation.** The P1-3 fix barred executed rest and permitted "as-written prescribed rest" instead — but **no rest column exists anywhere in the schema** (`007_program_engine.sql`, `022_set_target.sql`, `001_mechanical_input.sql` all lack one); rest is computed at execution time by `restSecondsFor` and never persisted. Under the missingness rule, every logged set would have been ineligible and **M0 would collapse to N = 0** | **FIXED** — preceding rest interval removed from M0 **and** M1 entirely (the panel's recommended Option A). Set-level context is carried by relative load, reps, set index and movement class. Reinstatement would require a persisted column plus fresh pre-registration |
| P1-NEW-2 | **The fifth door — adaptive execution.** `session_plan_slot.provenance_kind` admits `'substituted'`, `'day_swapped'`, `'added'`, `'free_form'` (`022_set_target.sql:46`). Substitution and day-swapping are often reactive to how the athlete feels, so they carry an unrecorded readiness signal — and scoring the *planned* movement instead mismatches features to a movement never performed | **FIXED** — the primary paired ablation is restricted to `provenance_kind = 'planned'`; adaptive sets go to a labelled secondary exploratory cohort, never pooled into the headline delta, with the excluded share reported per arm and per athlete |
| P2-NEW-1 | `09` §8 still said citation verification failed across "three rounds" — missed by **both** the B-3 and P2-5 sweeps | **FIXED** — corrected to four, with the repeat sweep failure disclosed in-line |
| P2-NEW-2 | `00` §4 cited `01` §12 (Perceived stress) for the construct-separation law; the non-conflation matrix is §13 | **FIXED** |
| P2-NEW-3 | **Trigger-maintained label duplicates.** Barring `session.session_rpe` was insufficient while `mech_daily.rpe_x_reps`, `reps_with_rpe` and `hard_sets` remain joinable — `trg_set_record_ai` maintains them synchronously (`001_mechanical_input.sql:96-126`), so a weighted mean RPE is recoverable and `hard_sets` approximates the high-effort outcome itself | **FIXED** — all three, plus any daily rollup derived from set target labels, added to the deny-list |

**Still not an approval.** The Round 2 verdict binds `920f49f` only, and the panel has not re-reviewed *this* remediation. Across CodeRabbit, Reviewer A (with a targeted re-verification), Reviewer B, and two Antigravity panel rounds, **no review has ever approved any state of this branch.**
