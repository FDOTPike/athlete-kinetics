...
Governing principle: the app collects three optional, day-scale, device-derived quantities today because they feed a ratified readiness feature [R01, R01b]. Everything in this document starts from that minimal footprint and holds it. Nothing here changes product behavior; this is the privacy/safety specification future work must satisfy.
...
- Per-type, per-feature justification [D02, D04]: each of the three types is consumed by the materialized readiness score [R01b]. A future data type must have the same property — a named consuming feature, reviewed before the permission request is ever added — or it must not be requested. "We might use it later" is not a justification.
...
- The three types may feed **one purpose only**: the readiness score and its trend disclosures [R01b]. They must never: influence set RPE or session RPE (including as hints), enter Coach-evidence deltas, gate sessions, or trigger safety halts. Calibration Policy v1 already stripped ACWR/load from readiness for exactly this class of reason [R01]; the same wall applies to every biometric here.
...
- Draft Health Connect permission wording (for any future declaration; not filed now): "Athlete Kinetics reads your nightly heart-rate variability (HRV), resting heart rate, and sleep data to calculate your daily readiness score on this device. Nothing is uploaded. You can disconnect at any time in your profile." Wording rules: name every type; name the feature; name the locality; name the revocation. No generic "health data" phrases [D02].
...
- **Retention:** daily rows for the trailing analytics windows; the store already re-materializes a bounded window and treats older rows as descriptive history [R01b]. There is no reason ever to retain raw ticks (none are stored). A future retention policy must be explicit, not an accident of query windows.
- **Deletion:** biometric rows must die with the athlete's training data. The existing `resetTrainingData` path is the deletion hook; any future biometric table must be registered there. Export behavior, if ever added, must carry the same consent framing as collection (export = disclosure risk).
