...
Governing principle: the app ingests three optional, day-scale, device-derived quantities. **Corrected data flow (per audits, against the live repository):** HRV and sleep efficiency feed the materialized readiness score [R01b]; the readiness score is **not merely descriptive** — it is a prescription input. `evaluatePolicy` (`packages/inference/src/policyReference.ts:36-55`) maps readiness (plus HRV z and sleep efficiency, which it also reads directly as rule conditions) to a load modifier (0.85–1.05), a set modifier (−2…+1), and a planned-RPE cap (6.5–9.5). Resting heart rate's exact current handling (**restated per third audit**): RHR-only data updates an existing `hrv_daily` row and is otherwise discarded (`useStore.ts:4237`); RHR has **no athlete-facing readiness/prescription consumer**; it is counted by the internal `hrvDays` diagnostic (`coachVerificationLab.ts:376`). Everything in this document starts from that actual footprint, not an idealized one.
