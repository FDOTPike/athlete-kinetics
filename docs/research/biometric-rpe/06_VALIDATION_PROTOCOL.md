...
- **Prediction target definition (pre-registered):** set-level RPE as a continuous value is the primary target. A coarse three-band classification (≤7.5 / 8–8.5 / ≥9, matching the app's cue-band structure [R05]) is a **secondary** target, justified only by [S03]'s degradation-away-from-failure and the app's existing band copy — **not** by any ML literature (the audit corrected the first freeze's "per the [S12] ceiling" justification: S12 is a two-class result at 2-minute windows and supports no per-set granularity claim, `02` §4b/§6).
...
- **Splits:** Arm P — athlete-disjoint train/holdout, split seed recorded before analysis. Arm I — chronological within-athlete calibration window → future-session holdout (§2). Every reported number names the arm that produced it.
- **Baselines (must precede any ML; evaluated within their applicable arm):** (a) population mean RPE per movement class (Arm P); (b) planned target RPE as naive predictor (both arms); (c) athlete's own historical mean RPE per movement, fitted on the calibration window only (Arm I — by construction unavailable in Arm P); (d) contemporaneous-RIR models only under a pre-registered leakage-free calibration procedure (§2 leakage rules), otherwise excluded. An advisory candidate must beat the strongest applicable baseline **within its arm** — per athlete in Arm I, with subgroup visibility in Arm P.
- **Model classes:** athlete-agnostic regularized regression on day-scale features (prior-night HRV, sleep efficiency, prior-day tonnage, movement class) for Arm P; the same model class with per-athlete calibration parameters fitted on the calibration window only, applied forward, for Arm I. This is the only class consistent with the app's closed-form deterministic posture [R01b] and the pilot's data density; any complex model is exploratory-only and reported as such.
- **Metrics (per arm; per athlete and pooled):** MAE and bias on continuous set RPE; band confusion for the secondary coarse-band target; calibration (predicted vs observed frequencies); high-effort miss rate (fraction of athlete-reported ≥9 sets predicted below 8 — the false-reassurance direction); false-reassurance rate at ≥8.5; uncertainty-interval coverage.
