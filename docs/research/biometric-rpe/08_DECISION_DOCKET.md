...
| UD-9 | Resting-heart-rate consuming-feature gap: drop the RHR request, give it a real consuming feature, or document an explicit justification — before any Health apps declaration is filed. **Accurate current handling (restated per third audit):** RHR-only data updates an existing `hrv_daily` row and is otherwise discarded (`useStore.ts:4237`); RHR has no athlete-facing readiness/prescription consumer; it is counted by the internal `hrvDays` diagnostic (`coachVerificationLab.ts:376`) [R09] — a developer-facing diagnostic count does not satisfy Google's user-facing-benefit test [D02, D05] | Francis Pike |
