...
- **Why it must not be conflated:** velocity is an external kinematic signal, not a body sensor; even if it were available it would estimate RIR, not replace the athlete's report. **Boundary, stated once and identically across this package (aligned per Round 4 review):** *shipped product code* may not generate, display, pre-fill or grade a biometric-derived RPE — unconditionally; the *offline research pilot* of `06` may compute predictions **solely for blinded validation**, with no athlete-facing and no prescription authority (`00` §1); and **existing readiness-driven planning is unchanged** — HRV and sleep continue to modulate the *planned* prescription through `readiness_score` and directly in `evaluatePolicy` [R07], which is a planning path and never an RPE path (§13.3). "No prescription authority" in this package always means *no new biometric authority over athlete-reported effort*, never a claim that readiness stopped affecting planning.
