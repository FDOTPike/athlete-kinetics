...
## 4b. Direct resistance-training RPE/fatigue estimation research (added in remediation)

The first freeze missed the direct resistance-training literature below. The independent audit required these to be reviewed and explicitly bounded before any negative evidence-landscape conclusion. All three are now verified against their primary pages; none changes the decision token, but all three change what "no evidence" is allowed to mean.

**[S25] PERSIST — a public multimodal resistance-training RPE dataset (the closest direct match).**
Schumann M, Wehner M, Düking P, Zakharkina D, Sperlich B (author list per publisher page, MDPI), "PERSIST: A Multimodal Dataset for the Prediction of Perceived Exertion during Resistance Training," Data 2023;8(1):9; doi 10.3390/data8010009; published 2022-12-28. Sixteen young healthy men screened, twelve consenting to open data; twelve flywheel-squat sets × 12 reps with lactate-confirmed fatigue; **set-level RPE reported after each set**; synchronized IMU + ECG + motion capture + HRV parameters. What it shows: per-set RPE prediction from body-worn sensors is an active, resourced research direction with a public dataset. What it does not show: any validated product-grade estimator; a single exercise on a lab flywheel platform; a small homogeneous male cohort; lab instrumentation (ECG, MoCap) far beyond what any consumer wearable or phone provides. Applicability bound: demonstrates feasibility *research*, not consumer-device validity; directly informs the `06` pilot design (labels per set; lactate-anchored fatigue substudy analog).

**[S26] Computer-vision RPE prediction during resistance training.**
Wei et al., "Hard set: a personalized RPE prediction model based on workout data using computer vision," Biomedical Signal Processing and Control 2024;89:105701; doi 10.1016/j.bspc.2023.105701 (identity via publisher/DOI registry; publisher page bot-walled at remediation time — full text not extracted, numbers cited from the audit's review and marked UNRESOLVED for quantitative use here). Camera-based RPE prediction during resistance exercise — again a lab/vision modality the phone-only app does not have, outside the consumer-biometric scope of this discovery's core claim.

**[S27] ML fatigue estimation during repeated isokinetic bench press (RPE-labeled).**
Sensors 2025;25(20):6588 (PMC12609798; article ID sensors-25-06588; author list not extracted this run — cite by PMC/DOI): 32 healthy men (age 33.2±5.9), XIM isokinetic robotic bench press, load-cell force–time features paired with RPE labels; RF reached R² 0.84, MAE 0.62 RPE, 93.1% of predictions within ±1 RPE. What it shows: with a robotic system's *embedded force sensor* (a device-integrated mechanical measurement, not a body-worn biometric), continuous fatigue/RPE estimation reaches ~±0.6 RPE MAE. What it does not show: anything about wrist HRV/SpO2-class signals, free-weight settings, or phone-only collection. Applicability bound: the ±1-RPE error scale reported here is achieved with instrumentation the Athlete App will never have at set time; it strengthens — not weakens — the decision to treat consumer-biometric per-set estimation as unproven, and it sets a reference point for what a genuinely instrumented future could look like.

**Bounded negative conclusion (replaces the first freeze's overreach):** the direct resistance-training literature now on file ([S25], [S26], [S27]) establishes that per-set RPE/fatigue prediction from sensors is a **live research area with promising lab results** — under ECG/MoCap/load-cell/camera instrumentation, small homogeneous cohorts, and single-exercise protocols. What remains unproven is the specific claim this discovery needed to test: that **consumer-watch/phone biometrics** (the modality this app could actually collect, `04`) can estimate per-set resistance-training RPE with athlete-level validity. The evidence gap is therefore *modality-and-setting-specific*, not a general absence of research. The first freeze's claim that S12 was "the strongest direct match" and the preprint "the only near-match" was wrong; the negative conclusion is now stated only at that bounded scope (`09` §4.2 updated accordingly).
...
- **Direct resistance-training estimation exists and is promising in the lab:** set-level RPE prediction on public multimodal data ([S25] PERSIST), camera-based personalized RPE prediction ([S26]), and load-cell fatigue/RPE estimation reaching MAE ≈ 0.62 RPE / 93.1% within ±1 RPE on a robotic bench press ([S27]).
- **None of it validates the modality this app could actually collect:** every direct study relies on instrumentation beyond consumer reach (chest ECG + motion capture, camera systems, embedded robotic load cells), small homogeneous cohorts, and single-exercise protocols.
- **The consumer-modality ceiling remains S12** — a two-class (high/low exertion, RPE 3.5 threshold), 2-minute-window, n=10 cycling classification (best 85.7% KNN) — which is not a per-set resistance-training result.
...
**Corrected verdict:** it is unproven — not disproven — that consumer-watch/phone biometrics can estimate per-set resistance-training RPE with athlete-level validity. The evidence base is too thin and too instrumented-lab-bound to justify a product estimate, and equally too active to justify the stronger NO-GO claim that the direction is unscientific. This is exactly the question the `06` pilot is designed to test, and why the token is RESEARCH PILOT ONLY.
